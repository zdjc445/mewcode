...
| thinking | 只透传真实分片；仅工具调用轮保存完整协议元数据并回传 Provider |
...
16. 保存工具调用轮的完整 thinking 协议元数据，满足后续 Provider 请求的协议要求。
...
- `ThinkingBlock.text` 必须非空。
- `ThinkingBlock.signature` 必须是字符串；OpenAI 协议使用空字符串。
- 只有包含非空 `tool_calls` 的 assistant 消息可以保存 `thinking_blocks`。
- user、tool 和无工具调用的 assistant 消息不得保存 `thinking_blocks`。
- `ConversationHistory.add_assistant_tool_calls()` 增加仅限关键字参数 `thinking_blocks`。
- thinking 不得拼入普通 assistant `content`。
...
- 每个完整 thinking 块输出一个 `ProviderThinkingComplete`。
...
- `ProviderThinkingDelta` 立即转换成 `ModelThinkingEvent`，只用于过程展示。
- `ProviderThinkingComplete` 不转换成 Agent 对外事件，只由 `AgentLoop` 收集。
- 本轮存在工具调用时，完整 thinking 块随 assistant 工具调用消息写入历史。
- 本轮没有工具调用时，完整 thinking 块在本轮结束时丢弃，不写入历史。
- OpenAI Provider 从真实 `delta.reasoning_content` 产生 thinking 分片；工具调用 assistant 消息把各块 `text` 按顺序拼接为 `reasoning_content` 回传。
- Anthropic Provider 从真实 thinking content block 产生分片和完整块；工具调用 assistant 消息按原顺序回传 thinking content block，并保留每块 `signature`。
- thinking 不得转换成 `ProviderTextDelta`、`ModelTextEvent` 或普通 assistant 正文。
...
7. thinking 和正文分片立即转换为 Agent 事件，完整 thinking 块只在本轮内收集。
8. 在 `ProviderTurnEnd` 前只收集内容，不执行工具。

### 10.3 有工具调用

1. 如果当前是第 `15` 轮，终止为 `max_rounds_exceeded`，不得执行工具。
2. 把完整 assistant 正文、完整 thinking 块和工具调用写入历史。
...
- assistant 工具调用消息必须在对应工具结果之前写入，并保存该轮完整 thinking 块。
- 每个工具结果必须使用准确的 `tool_call_id`。
- 多工具结果必须按模型原始调用顺序写入。
- 最终正文必须在发出 `FinalResponseEvent` 前写入 assistant 历史。
- 无工具调用轮的 thinking 不写入历史。
- 工具调用轮的 thinking 只存放在 `thinking_blocks`，不得混入 assistant `content`。
...
- 无工具调用轮的 thinking 不进入历史。
- 工具调用轮的完整 thinking 作为 `thinking_blocks` 进入历史，普通正文不包含 thinking。
- 每次请求独立应用 `15` 轮上限。
- 规划和获批执行共用轮数。
- 第 15 轮不传工具并加入收尾提示。
...
- OpenAI 工具调用轮保存并回传 `reasoning_content`。
- Anthropic 工具调用轮保存并回传 thinking block 及其 `signature`。
- 无工具调用轮不保存 thinking。
- 多工具参数分片组装。
- 停止原因归一化。
...
14. thinking 分片只作为过程事件输出；完整 thinking 仅在工具调用轮作为协议元数据进入历史。
15. 两个 Provider 都输出统一结构化流事件。
16. OpenAI 工具调用轮回传 `reasoning_content`，Anthropic 工具调用轮回传 thinking block 和原始 `signature`。
17. 无工具调用轮的 thinking 不进入历史。
18. 所有错误消息脱敏。
19. 默认测试不访问网络。
20. 以下命令全部成功：
