...
隔离 Agent 在 `load_skill` 工具内部运行，现有 Tool 接口不能把新的工具确认请求转发到外层 TUI。因此隔离 run 遇到 `ToolApprovalRequestedEvent` 时固定拒绝该次内部调用，随后允许 Agent 根据拒绝结果继续；不得静默授权或等待一个无法到达用户的确认。被安全策略、session 规则或永久规则直接允许的调用不会产生该事件，仍可正常执行。
